...
| I4d | Single-item plays (Cloud/downloaded/Last-Played) don't auto-advance — enqueue surrounding tracks | M | [x] Spotify-autoplay-off DEVICE-VERIFIED 2026-08-03 (vc86): user found a pinned Spotify album kept advancing with autoplay OFF (Spotify Connect's own queue, not the local player) → NEW `startSpotifyAutoplayGuard` pauses on an auto-advance boundary; device: Foreword end → `[AUTOPLAY] … → pause`, Spotify PAUSED; negative test (mid-track user skip) NOT paused. Residual: LOCAL single-item enqueue code-confirmed + mechanism device-proven, full LOCAL test needs a multi-track non-Spotify pinned context (content-gated) |
