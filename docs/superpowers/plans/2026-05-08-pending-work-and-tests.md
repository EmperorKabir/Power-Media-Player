...
|---|---|---|---|
| C7 Per-file overrides | LOCKED — long-press starred/pinned → 3-tab popup (Audio/Video/Speed) → Room `media_overrides` → auto-apply at play start → indicator chip | ⚠️ **shipped in `00d5fa2`** — Room v7→v8 migration + `MediaOverrideEntity` + DAO + `MediaOverridesPopup` (3 tabs) + Library / LastPlayed call-site wiring + apply-on-play hook in `PlayerViewModel`. **Partial:** speed/pitch/volume-boost apply live; per-file reverb / stereo-flip / mono-mix / video-flips / BW / sepia / invert / rotation persist to DB but don't yet feed the effect chain (needs a "transient override" channel on the global effect pipeline). Indicator chip in player UI is owed. | follow-up: wire transient effect channel + indicator chip |
| C25 Long-press menu Override-* items | LOCKED — items appear when row is starred/pinned | ✅ **CLOSED in `00d5fa2`.** Library wires Override-* when row is favourited; LastPlayed wires when row is pinned. Cloud deliberately deferred (Spotify off-pipeline; Drive round-trips via pinned). | grep `TrackContextActions(` confirms callbacks now passed at LibraryScreen.kt + LastPlayedScreen.kt |
