    ...
    sections = listOf(
        ...
        InfoSection(
            ...
            bullets = listOf(
                ...
                "New episode alerts: turn on Notify on new episode in a podcast's settings and the app posts a notification when a background sync finds fresh episodes. Android asks for notification permission the first time you switch it on. Choose or silence the alert sound in system settings under App notifications, New podcast episodes.",
                "Cover art while browsing: audio files show their real embedded cover in the list, even before you download or favourite them. The app fetches a small piece of each file to read the picture and remembers the result, so nothing is fetched twice. On mobile data this waits for Wi-Fi unless you turn on 'Download cover art on mobile data' in Settings, Cloud. A downloaded copy always provides the final cover.",
                "Mobile data control: 'Download files on mobile data' in Settings, Cloud governs every large transfer, that is saving files for offline use, podcast episode downloads including automatic ones, and the full metadata fetches for cloud audiobooks. When it is off those wait for Wi-Fi and the app tells you when a download was blocked. Streaming, browsing, casting and podcast feed updates always work on any connection. Android treats all mobile data as limited, even unlimited plans, so leave it on if your plan is unlimited.",
                "Search: searches inside the active provider only. Spotify catalogue search returns tracks, albums, artists, playlists, shows (podcasts) and single episodes. Result rows show 'Title, Artist' (or show publisher). Tap an artist to drill into their top tracks; tap a standalone episode to play the full episode on your Spotify Connect device (Premium). Drive search looks inside the folders you have added, by file name and by the title or author of items you have already opened or favourited (so searching an author like \"Matt\" finds the book even when the file name does not). It does not search your whole Drive.",
                ...
                "Auto-refresh on return: the Cloud tab refreshes on every resume (for example after signing in to Drive or Spotify). Earlier builds could get stuck on the sign-in cards even after a successful pick; this is fixed.",
