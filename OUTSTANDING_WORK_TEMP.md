...
=====================================================================
- [c] Enrich-on-download — DONE in code + built (vc69) + installed. Fires DriveTagEnricher.enrich after offlineCopyDao.upsert in OfflineMediaManager.downloadDrive (reuses local copy via offlineCopyDao.get(id).localPath — no re-download; writes enrichment_cache → downloaded-row subtext populates). Both @Singleton, no Hilt cycle. Rests on the SAME enrich→cache→row path that device-renders "This Inevitable Ruin" author subtext. ON-SCREEN capture blocked: the Oppo keyguard engaged when the sleep-test screen-off fired (secured lock, adb can't dismiss). RE-VERIFY once unlocked: download an un-favourited Drive file → §1 row shows Artist/Author subtext.
