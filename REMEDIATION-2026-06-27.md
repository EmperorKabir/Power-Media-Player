...
- **M3 backup** — FUNCTIONAL round-trip: UI renders 4 actions; "Back up to Drive" → "Backed up to Drive."; "Restore from Drive" → "Restored 2493 item(s)." FK-safe two-pass restore handled 2493 real rows. **VALUE-CORRECTNESS device-verified 2026-06-29 (re-audit):** on-device SHA-256 of `files/datastore/settings.preferences_pb` across a Drive round-trip — A=`0dd55b90…` (pre-backup) → back-up (settings unchanged, still `0dd55b90`) → flipped the Pre-fetch toggle → B=`4807b646…` (changed) → Restore from Drive → C=`0dd55b90…` = **byte-identical to A**. So restore returns exact settings values, not just row counts. (Pre-fetch toggle + Drive backup restored to the user's real state afterwards.) **Local-SAF leg ALSO device-verified 2026-06-29** (re-audit follow-up — the system DocumentsUI picker IS adb-navigable, unlike the WebView Drive picker): "Back up to file" → DocumentsUI → SAVE → "Backup saved." (`PowerMediaPlayer-backup-20260629-023815.json`); flipped Pre-fetch (hash `4807b646→0dd55b90`); "Restore from file" → picked the file → "Restored 2496 item(s)." → settings hash back to `4807b646` = **byte-identical**. BOTH legs (Drive + local-SAF) now value-verified. (DB-table byte-diff confounded by timestamps; covered by BackupManagerTest + the row-count round-trip.)
- **Override-on-resume (audio/video/EQ)** — gap closed by code architecture (2026-06-29 re-audit): the effect axes (stereoFlip/monoMix/voiceBoost/reverb/skipSilence/EQ) are applied by CONTINUOUS reactive collectors keyed on the current item's override (`PlaybackService.kt:900-916` `mediaOverrideRepo.withOverrideBool(...).collect{ flag=it }`) — the SAME path used during normal playback (device-proven). Cold-start resume merely makes the restored item current → those collectors apply its override identically. There is NO separate resume code path for effects that could silently fail; only speed/pitch use a distinct cold-start path, and that is already device-verified (`Override direct axes: speed=1.2 pitch=0.8`).
- **M5 podcast download** — CONSTRAINT verified: `dumpsys jobscheduler` shows the live `#PodcastSyncWorker#` job carries `NET satisfied:0x13700000` = the `CONNECTED` constraint (wifi+mobile), not UNMETERED. Device has BOTH a WIFI and a CELLULAR network. GAP (2026-06-29 re-audit): the end-to-end metered *download* was NOT forced — the WorkManager job is namespaced (`androidx.work.systemjobscheduler:u0a654/0`) so `cmd jobscheduler run -f com.powermediaplayer 0` returns "Could not find job 0"; confirming the metered download needs a real subscribed auto-download show with a pending episode while on mobile data (wifi off).
...
- **M4 sleep-timer REMOTE fade (CAST)** — DEVICE-VERIFIED 2026-06-29 on Living Area TV after a real bug was found + fixed:
  ...
  - SPOTIFY remote fade — **DEVICE-VERIFIED 2026-06-29** (re-audit): started Spotify myself (phone = "This phone" Connect device; PMP cold-start re-mirrored it — `Cold-start Spotify resumed … session 13`, poll active). Added a `SPOTFADE` diag mirroring CASTFADE (commit ba3c611). 1-min TIME_BASED timer — the custom field is a **SeekBar** (min 1 min, drag-to-start), NOT the unreliable text path I'd assumed. Diag: `capture vol=100` → smooth ramp `set pct=76→73→70→…→10→6→3` over ~23 s → `restore vol=100` at expiry → `SleepTimer expired — paused playback (fadeOut=true)`. So the Spotify Connect volume ramps correctly via the conflated-channel `setVolume` PUTs, restores the captured base, and pauses at expiry (HIGH audit fix 4f4c07d confirmed live). NO bug, unlike cast. Both remote routes (Cast + Spotify) verified on the TIME_BASED loop; the END_OF_* loops share the same `applyFadeFactor` mechanism. Local path unaffected.
