...
| T293 | "2 media players" status-bar conflict (Spotify + video both playing) | I | DONE-INVESTIGATION(convicted: cold-start auto-resume FORCE-LAUNCHES the phone's Spotify app via BounceBridge when no Connect device exists — playTrackOnConnectDevice:661 launchSpotifyAndReturn — creating a 2nd media notification alongside PMP. Local-play paths already pause+stopPolling Spotify (Library/LastPlayed/Cloud VMs). FIX is a product choice: aggressive force-launch vs gentle resume-only-if-device-available) AWAITING-USER(choose auto-resume aggressiveness) |
